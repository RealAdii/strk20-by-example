...
// Vendored from React Bits (JS + CSS variant), ported to TS.
// Local changes: typed props, plus a `paused` prop so the hero can honour
// prefers-reduced-motion. Upstream already pauses when offscreen or when the
// tab is hidden; this adds a third reason to stop.
...
const Grainient: React.FC<GrainientProps> = ({
  ...
  // Effect 1: build the WebGL context once; pause offscreen / tab hidden / paused
  useEffect(() => {
    ...
    const tryStart = () => {
      if (isVisible && isPageVisible && !isPaused && raf === 0) {
        raf = requestAnimationFrame(loop)
      }
    }
    const tryStop = () => {
  ...
  // Effect 3: honour `paused` without tearing the context down
  useEffect(() => {
    const container = containerRef.current
    if (!container) return
    ctxMap.get(container)?.setPaused(paused)
  }, [paused])
