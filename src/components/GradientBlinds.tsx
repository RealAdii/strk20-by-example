...
// Local changes: typed props, no "undefined" leaking into className.
...
const GradientBlinds: React.FC<GradientBlindsProps> = ({
  ...
  // Read `paused` through a ref so toggling it never tears down the context.
  useEffect(() => {
...
`
    ...
    // The canvas sits behind the hero copy and is pointer-events:none, so the
    // spotlight tracks the pointer from the hero wrapper instead.
    const pointerTarget: HTMLElement = container.parentElement || container
  ...
  return (
    <div
      ...
      className={
        className
          ? `gradient-blinds-container ${className}`
